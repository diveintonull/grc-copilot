...
| 受控实验 | 观测结果 |
|---|---|
| 父子分块与固定窗口对比 | 在相同 Dense、无 Rerank 配置下，Recall@5 从 50.00% 提高到 69.15% |
| 对父子证据使用 Rerank | Recall@5 达到 81.91% |
| 渐进式 Skills 与加载全部 SOP 对比 | 平均输入 Token 减少 59.46%，固定 12 题 Skill 子集结构成功率达到 91.67% |

这些是限定口径的消融结果，不是生产准确率承诺。可复现的评测数据集、指标和运行器保留在 `evals/` 中。
